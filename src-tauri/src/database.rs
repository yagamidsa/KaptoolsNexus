// src-tauri/src/database.rs - VERSIÓN COMPLETA CON TODOS LOS FIXES
...
// FUNCIÓN 1: validate_database_connection - CON FIX
#[command]
pub async fn validate_database_connection(db_path: String) -> Result<DatabaseResponse, String> {
    ...
    // SCOPE para asegurar que la conexión se cierre
    let result = {
        match sqlite::open(&db_path) {
            Ok(connection) => {
                ...
                let query_result = match connection.prepare("SELECT COUNT(*) as user_count FROM usage_data") {
                    Ok(mut statement) => {
...
// FUNCIÓN 2: get_current_user - NO NECESITA CAMBIOS (no usa DB)
#[command]
...
// FUNCIÓN 3: update_user_connection - CON FIX
#[command]
    ...
    let result = {
        match sqlite::open(&db_path) {
            Ok(connection) => {
                ...
                let operation_result = match connection.prepare(check_query) {
                    Ok(mut statement) => {
...
// FUNCIÓN 4: get_dashboard_data - CON FIX
#[command]
    ...
    let result = {
        match sqlite::open(&db_path) {
            Ok(connection) => {
                ...
                let query_result = match connection.prepare(query) {
                    Ok(mut statement) => {
                    ...
                    Err(e) => {
                        ...
                        Err("Failed to prepare dashboard query".to_string())
                    }
                };
...
// FUNCIÓN 5: initialize_user_session - CON FIX
#[command]
pub async fn initialize_user_session(db_path: String, username: String) -> Result<String, String> {
    ...
    // Simplemente llamar a update_user_connection (que ya tiene el fix)
