...
const ReviewBranches: React.FC<ReviewBranchesProps> = ({ workspacePath, onClose }) => {
    ...
    const validateAndLoadBranches = async () => {
        ...
        try {
            ...
            setAvailableRepos(validateData.validation.repositories);
            ...
            if (!branchesRes.ok) {
                throw new Error(`HTTP ${branchesRes.status}: ${branchesRes.statusText}`);
            }
            ...
            if (branchesData.success) {
                ...
                setSuccess(`Found ${branchesData.total} branches`);
            } else {
    ...
    const handleRefresh = async () => {
        ...
        try {
            ...
            console.warn('Fetch failed, but continuing with local branches');
        }
    ...
    const FileDiffViewer = () => {
        ...
        return (
            <div className="file-diff-overlay" onClick={(e) => {
